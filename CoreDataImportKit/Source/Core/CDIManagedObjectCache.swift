...
open class CDIManagedObjectCache {
     ...
     */
    open func buildCacheForExternalRepresentation(_ externalRepresentation: CDIExternalRepresentation, usingMapping mapping: CDIMapping) {
    ...
    */
    open func managedObjectForRepresentation(_ representation: CDIRepresentation, usingMapping mapping: CDIMapping) -> NSManagedObject? {

        let cache = objectCacheForMapping(mapping)

        // TODO: See if there is a more Swift way to do this
        if let primaryKeyValue = mapping.primaryKeyValueFromRepresentation(representation) as? NSObject {
    ...
    /**
     ...
     - returns: Managed object, if one exists
     */
    open func managedObjectWithPrimaryKey(_ primaryKeyValue: NSObject, usingMapping mapping: CDIMapping) ->  NSManagedObject? {
        let cache = objectCacheForMapping(mapping)
        return cache[primaryKeyValue]
    }
     ...
     */
    open func addManagedObjectToCache(_ managedObject: NSManagedObject, usingMapping mapping: CDIMapping) {

        if let primaryKeyValue = mapping.primaryKeyValueForManagedObject(managedObject) as? NSObject, let object = objectCache[mapping.entityName] {
            var obj = object
            obj.updateValue(managedObject, forKey: primaryKeyValue)
        }
    ...
    func buildCacheForRepresentation(_ representation: CDIRepresentation, usingMapping mapping: CDIMapping) {
        if let primaryKeyValue = mapping.primaryKeyValueFromRepresentation(representation) as? NSObject {
        ...
        for (_, relationshipDescription) in mapping.relationshipsByName {
            ...
            if let entity = relationshipDescription.destinationEntity,
                let destinationEntityName = entity.name,
                let destinationEntityMapping = mapping.mappingForRelationship(relationshipDescription),
                let representationValue = mapping.valueFromRepresentation(representation, forProperty: relationshipDescription) {
                    ...
                    else if let representationValue = representationValue as? NSObject {
    ...
    func objectCacheForMapping(_ mapping: CDIMapping) -> [ NSObject : NSManagedObject ] {
    ...
    func fetchExistingObjectsForMapping(_ mapping: CDIMapping) -> [ NSObject : NSManagedObject ] {

        let entityName = mapping.entityName

        // Make sure we have the primaryKeys and the primaryKey for the entity in question
        guard let primaryKeys = primaryKeyValuesCache[entityName], let primaryKeyAttributeName = mapping.primaryKeyAttributeName else {
            return [:]
        }
        ...
        do {
            ...
            for object in existingObjects as! [NSManagedObject] {
                ...
                // TODO: See if there is a better "Swift" way of doing this. Check for String or Int?
                if let primaryKeyValue = mapping.primaryKeyValueForManagedObject(object) as? NSObject {
