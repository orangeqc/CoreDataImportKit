...
open class CDIManagedObjectCache {
    ...
    */
    open func managedObjectForRepresentation(_ representation: CDIRepresentation, usingMapping mapping: CDIMapping) -> NSManagedObject? {
        ...
        if let primaryKeyValue: NSObject = mapping.primaryKeyValueFromRepresentation(representation) as? NSObject {
    ...
    open func addManagedObjectToCache(_ managedObject: NSManagedObject, usingMapping mapping: CDIMapping) {
        ...
        if let primaryKeyValue = mapping.primaryKeyValueForManagedObject(managedObject) {
            _ = objectCache[mapping.entityName]?.updateValue(managedObject, forKey: primaryKeyValue)
        }
    ...
    func buildCacheForRepresentation(_ representation: CDIRepresentation, usingMapping mapping: CDIMapping) {
        if let primaryKeyValue = mapping.primaryKeyValueFromRepresentation(representation) {
            ...
            primaryKeyValuesCache[mapping.entityName]?.insert(primaryKeyValue as! NSObject);
        }
            ...
            if let entity = relationshipDescription.destinationEntity,
                ...
                let representationValue = mapping.valueFromRepresentation(representation, forProperty: relationshipDescription) {
                    ...
                    if representationValue is CDIRepresentationArray || representationValue is CDIRepresentation {
                        buildCacheForExternalRepresentation(representationValue as CDIExternalRepresentation, usingMapping: destinationEntityMapping)
                    }
                    ...
                    // To-one where the representation only has the foreign key
                        ...
                        // Add primary key to the cache
                        primaryKeyValuesCache[destinationEntityName]?.insert(representationValue as! NSObject)
                    }
